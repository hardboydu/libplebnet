...
' < $1 | awk "
	...
	type("STD") || type("NODEF") || type("NOARGS") || type("NOPROTO") \
		...
		if (!flag("NOPROTO") && !flag("NODEF")) {
                    if (funcname == "nosys" || funcname == "lkmnosys" || funcname == "sysarch") {
                          printf("%s\t%s(struct thread *, struct %s *)",
			    rettype, funcname, argalias) > sysdcl
                    } else if (funcname ~ /^freebsd/ || funcname ~ /^linux/ || funcname ~ /^svr4/ || funcname ~ /^ibcs2/ || funcname ~ /^xenix/) {
                          printf("%s\t%s(struct thread *, struct %s *)",
			    rettype, funcname, argalias) > sysdcl
                    } else {
		...
		if (flag("NOSTD")) {
			...
			column = column + length("lkmressys") + length("AUE_NULL") + 3
                    ...
                    if (funcname == "nosys" || funcname == "sysarch" || funcname == "lkmnosys") {
			...
			column = column + length(funcname) + length(auditev) + length(flags) + 3 + 3
                    }
		} 
